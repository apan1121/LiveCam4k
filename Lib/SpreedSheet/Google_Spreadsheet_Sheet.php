...
class Google_Spreadsheet_Sheet {
  ...
  public function __construct ($id, $client) {
    ...
    $this->spreadSheet = $this->sheet->spreadsheets->get($this->id);
  }
  ...
  /**
  * 依col , row來取得sheet坐標，加入$to系列可連續取得
  *
  ...
  * @return void
  */
  public function get($name, $fromCol, $fromRow, $toCol = null, $toRow = null) {
    $to = false;
    ...
    return $this->sheet->spreadsheets_values->get($this->id, $range);
  }
